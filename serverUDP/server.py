...
#funzioni 
def checkPort(num): #controllo porta
	try:
		port = int(num)
...
def checkArgv(arg): #controllo argomenti che devo essere 3
...
def ServerList(): #funzione per la creazione della lista
    try:
        files = os.listdir()
        lists = []
...
def UDPServer(host, port): #funzione UDP server
    try:
        ...
        while True:
            ...
            print("server OK")
            time.sleep(1)
            if istr == "menu":  #apertura menu principale
                message = message + "\nUDP Server actions:\n" 
