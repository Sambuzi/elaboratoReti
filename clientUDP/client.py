...
def checkArgv(arg):    #controllo degli argomenti che devono essere 3
...
def sendMessage(message, sAddr): #funzione per invio messaggi
    t = True
...
def UDPClient(sAddr): #funzione UDP client
    try:
        istr = sendMessage("menu", sAddr) #apriamo il menu generale
        print(istr)
        time.sleep(0.5)
        ...
        while True:
            ...
            if istr[0] == "list" and len(istr) == 1: #se siamo nel caso "list"
            ...
            elif istr[0] == "exit" and len(istr) == 1: #se siamo nel caso "exit"
                msg = sendMessage(istr[0], sAddr)
                print(msg)
                sys.exit()
                    
                
            elif istr[0] == "get" and len(istr) == 2: #se siamo nel caso "get"
                msg = sendMessage(istr[0], sAddr)
            ...
            elif istr[0] == "put" and len(istr) == 2: #caso "put"
...
if __name__ == '__main__' : #main del programma , richiamiamo le varie funzioni.
    try:
